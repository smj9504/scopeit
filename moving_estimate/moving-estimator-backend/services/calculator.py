"""
Estimation Calculator Service
Core calculation logic for pack-out estimates
"""

from typing import Dict, List, Tuple, Any
from models.schemas import (
    RoomInput, QuickEstimateRequest, EstimateResponse, EstimateStatus,
    RoomsEstimateRequest, DetectedContentItem, RoomItemSummary, StagingType,
    SupplementItem,
)
from sqlalchemy.orm import Session
from models.database import Price, RoomPreset
import math
...
# Base box materials per room — covers miscellaneous contents not captured
# by AI item detection (drawer contents, shelf items, small decor, toiletries).
BASE_ROOM_MATERIALS = {
    "small":  {"box_small": 1},
...
HINT_MATERIAL_MAP = {
    ...
    # ── Electronics ──────────────────────────────────────────────────────
    # Most rooms have 1 TV at most; other electronics use medium boxes
...
class EstimateCalculator:
    ...
    def _load_prices(self):
        """Load prices from database into memory"""
        prices = self.db.query(Price).all()
        self.prices = {p.code: p for p in prices}
        # Create reverse lookup by key
        self.price_by_key = {}
    ...
    def _load_presets(self):
        """Load room presets from database"""
        presets = self.db.query(RoomPreset).all()
        self.presets = {p.key: p for p in presets}
    
    def get_price(self, code: str) -> float:
        """Get price by code"""
        return self.prices.get(code, Price(price=0)).price
    ...
    # ============================================
    # CONDITIONAL SUPPLEMENTS (replaces flat contingency)
    # ============================================
    # Each supplement has a trigger condition evaluated against job context.
    # Only triggered supplements appear on the estimate; users can toggle them off.

    SUPPLEMENT_DEFINITIONS = [
        {
            "key": "hidden_damage",
            "name": "Unforeseen Conditions — Concealed Contamination",
            "description": "Per IICRC S500 §12.3 — additional handling for concealed water/mold damage discovered during packing",
            "flat_amount": 150.00,  # Fixed fee, not percentage
        ...
        },
        {
            ...
            "flat_amount": 85.00,  # ~1.5 hrs at labor rate, regardless of item count
            "trigger": "high_value",
            "min_items": 3,
        },
        {
            ...
            "description": "Narrow stairwell / no elevator — additional carry time and crew fatigue allowance",
            "flat_amount": 120.00,
            "trigger": "upper_floor",
        },
        {
            ...
            "description": "Above-average contents volume requiring additional sort/stage time and equipment",
            "flat_amount": 95.00,
    ...
    def evaluate_supplements(
        ...
        for defn in self.SUPPLEMENT_DEFINITIONS:
            ...
            if defn["trigger"] == "contamination":
                ...
                triggered = ctx["high_value_count"] >= defn.get("min_items", 3)
            ...
            # Use user-provided amount override, otherwise use flat_amount
    ...
    def calculate_material_cost(self, materials: Dict[str, int]) -> float:
        ...
        for mat_key, qty in materials.items():
            ...
            if code and code in self.prices:
                total += self.prices[code].price * qty
        return total
    ...
    def calculate_estimate(self, request: QuickEstimateRequest) -> EstimateResponse:
        ...
        special_item_lines = []
        for item_key in getattr(request, 'special_items', []):
            spec = SPECIAL_ITEM_COSTS.get(item_key)
            if spec:
                special_item_cost += spec["price"]
                special_item_lines.append(spec)
        for custom in getattr(request, 'custom_special_items', []):
        ...
        sections = {
            ...
            "Supervision": supervisor_hours * supervisor_rate,
        ...
        # Evaluate conditional supplements (replaces flat contingency)
        ...
        # Legacy contingency support (for backwards compatibility)
        ...
        section_details["Pack-Out Labor"] = {"lines": [
            {"name": "Pack-Out Labor", "qty": pack_out_hours, "unit": "HR",
             "rate": round(labor_rate, 2),
             "detail": f"{pack_out_hours} person-hrs · {crew}-person crew  (room-based estimate: wrap, box, stage)",
             "amount": round(pack_out_hours * labor_rate, 2)},
        ]}
        section_details["Supervision"] = {"lines": [
            {"name": "Supervision", "qty": supervisor_hours, "unit": "HR",
             "rate": round(supervisor_rate, 2),
             "detail": f"{supervisor_hours} hrs × 1 supervisor  (quality control, crew coordination)",
        ...
        if request.include_packback:
            section_details["Pack-Back Labor"] = {"lines": [
                {"name": "Pack-Back Labor", "qty": pack_back_hours, "unit": "HR",
                 "rate": round(labor_rate, 2),
                 "detail": f"{pack_back_hours} person-hrs · {crew}-person crew  (unpack, place, remove packing material)",
                 "amount": round(pack_back_hours * labor_rate, 2)},
            ]}
    ...
    # Base minutes per item by category for labor estimation
    # ── Size-tiered labor minutes ──────────────────────────────────────
    # Minutes represent the TOTAL time for an item LINE regardless of quantity.
    # A "Framed Picture qty:5" gets the same time as qty:1 because the crew
    # wraps them together in one session — it's a per-LINE cost, not per-piece.
    #
    # For quantity > 1, a small marginal factor (QUANTITY_MARGINAL_FACTOR) is
    # applied: each additional unit adds ~20% of base time (stacking, labeling).
    #
    # Reference: Xactimate CPS BXSME = 9.6 min/box (pack only).
    # Insurance restoration adds photo + serial/condition → ~15–20 min/box.
    MINUTES_BY_SIZE = {
        "Furniture": {"large": 60, "medium": 35, "small": 15},
        "Electronics": {"large": 35, "medium": 20, "small": 10},
        "Books": {"large": 25, "medium": 15, "small": 8},
        "Kitchenware": {"large": 25, "medium": 15, "small": 8},
        "Clothing": {"large": 20, "medium": 12, "small": 6},
        "Fragile": {"large": 40, "medium": 25, "small": 12},
        "Artwork": {"large": 45, "medium": 30, "small": 15},
        "Collectibles": {"large": 35, "medium": 22, "small": 10},
        "Appliances": {"large": 55, "medium": 35, "small": 15},
        "Tools": {"large": 20, "medium": 12, "small": 6},
        "Sports": {"large": 30, "medium": 15, "small": 8},
        "Other": {"large": 18, "medium": 10, "small": 5},
    }

    # Each additional unit beyond qty=1 adds this fraction of the base time.
    # e.g., "Nightstand qty:2" = 15min + (15 * 0.20) = 18min total, not 30min.
    QUANTITY_MARGINAL_FACTOR = 0.20

    # Map specific canonical item names → size class.
    # Items not listed default to "medium".
    ITEM_SIZE_CLASS = {
        # Furniture — Large (disassembly, 2-person carry, >50 lbs)
        "King Bed Frame": "large", "Queen Bed Frame": "large",
        "Sectional Sofa": "large", "Sleeper Sofa": "large",
        "Wardrobe": "large", "Display Cabinet": "large",
        "Dining Table": "large", "Buffet Table": "large",
        "Desk": "large", "Workbench": "large",
        # Furniture — Small (single-person, <25 lbs, no disassembly)
        "Nightstand": "small", "End Table": "small",
        "Ottoman": "small", "Bar Stool": "small",
        "Coffee Table": "small", "Coat Rack": "small",
        "Shoe Rack": "small", "Dining Chair": "small",
        "Vanity Table": "small", "Filing Cabinet": "small",
        "Toddler Bed": "small",
        # Electronics — Large
        "Flat Screen TV": "large", "Desktop Computer": "large",
        "Sound System": "large",
        # Electronics — Small
        "Router/Modem": "small", "Gaming Console": "small",
        "Laptop Computer": "small", "Printer": "small",
        # Appliances — Large (requires dolly, disconnect utilities)
        "Refrigerator": "large", "Washing Machine": "large",
        "Dryer": "large", "Dishwasher": "large",
        "Window AC Unit": "large",
        # Appliances — Small
        "Microwave": "small", "Coffee Maker": "small",
        "Toaster": "small", "Blender": "small",
        "Air Fryer": "small", "Rice Cooker": "small",
        "Small Kitchen Appliance": "small",
        # Artwork — Large
        "Canvas Painting": "large", "Wall Mirror": "large",
        # Artwork — Small
        "Wall Clock": "small",
        # Sports — Large
        "Treadmill": "large", "Exercise Bike": "large",
        "Elliptical Machine": "large", "Weight Bench": "large",
        # Sports — Small
        "Yoga Mat": "small", "Sports Equipment Bag": "small",
        # Fragile — Large
        "Chandelier": "large",
        # Fragile — Small
        "Candle Holder": "small", "Decorative Bowl": "small",
        # Kitchenware — Small
        "Utensil Set": "small", "Mug Set": "small",
        "Food Storage Containers": "small", "Bakeware Set": "small",
        # Clothing — Small
        "Shoes": "small", "Hat Collection": "small",
        # Books — Small
        "Magazine Collection": "small", "DVD/Blu-ray Collection": "small",
        # Other — Small
        "Pillow": "small", "Bed Sheet Set": "small",
        "Towel Set": "small", "Fan": "small",
        "Trash Can": "small", "Garden Hose": "small",
        "Suitcase": "small", "Storage Bin": "small",
        "Cardboard Box": "small", "Basket": "small",
        # Other — Large
        "Area Rug": "large", "Safe": "large",
        # Tools — Large
        "Lawn Mower": "large", "Ladder": "large",
    }

    # Legacy flat fallback (kept for calculate_estimate quick path)
    MINUTES_PER_ITEM = {
        cat: tiers["medium"] for cat, tiers in MINUTES_BY_SIZE.items()
    }

    @classmethod
    def get_item_labor_minutes(cls, item_name: str, category: str, quantity: int = 1) -> float:
        """Get TOTAL labor minutes for an item line based on name, category, and quantity.

        Minutes are per-LINE, not per-piece:
        - Base time covers the first unit (setup, document, wrap, stage).
        - Each additional unit adds only QUANTITY_MARGINAL_FACTOR of base time
          (stacking, labeling — the crew is already set up).

        Example: "Framed Picture" (medium Artwork) qty=5
          base=30min, total = 30 + 4 * (30 * 0.20) = 30 + 24 = 54min
          vs. old logic: 30 * 5 = 150min (unrealistic)
        """
        size = cls.ITEM_SIZE_CLASS.get(item_name, "medium")
        tiers = cls.MINUTES_BY_SIZE.get(category, cls.MINUTES_BY_SIZE["Other"])
        base = tiers.get(size, tiers["medium"])
        if quantity <= 1:
            return base
        marginal = base * cls.QUANTITY_MARGINAL_FACTOR
        return base + (quantity - 1) * marginal

    # Per-room overhead in minutes (setup, 360° photo documentation, inventory start, final sweep)
    # Insurance restoration requires thorough photo documentation per room before packing starts.
    # Reference: industry standard allows 30–45 min per room for documentation + setup.
    ROOM_OVERHEAD_MINUTES = 40
    ...
    def _supplement_furniture_materials(
        ...
        if not is_large:
            return
        # Ensure at least 2 blankets per unit for large furniture.
    ...
    # Category-based material defaults for items lacking required_materials.
    # Keys match _normalize_material_key aliases (same format as AI output).
    # These represent what a single unit of each category typically needs.
    # Repeated keys indicate multiple units of that material per item.
    _CATEGORY_DEFAULT_MATS: Dict[str, List[str]] = {
        "Furniture":    ["moving_blanket", "moving_blanket", "stretch_wrap"],
        "Electronics":  ["bubble_wrap_12", "medium_box"],
        "Kitchenware":  ["packing_paper", "dish_pack_box"],
        "Fragile":      ["packing_paper", "bubble_wrap_12", "dish_pack_box"],
        "Books":        ["small_box"],
        "Artwork":      ["bubble_wrap_24", "mirror_box"],
        "Collectibles": ["bubble_wrap_12", "small_box"],
        "Clothing":     ["medium_box"],
        "Appliances":   ["moving_blanket", "stretch_wrap"],
        "Tools":        ["small_box"],
        "Sports":       ["large_box", "moving_blanket"],
    }
        ...
        for room in rooms:
            for item in room.items:
                ...
                if item.required_materials:
                    ...
                    # No AI-provided materials: fall back to category defaults
                    cat = getattr(item, 'category', None) or 'Other'
                    item_mats = self._CATEGORY_DEFAULT_MATS.get(cat, [])
                ...
                for mat_key in item_mats:
                    ...
                    if norm == "packing_paper":
                        ...
                        # Wrap materials: each occurrence = 1 unit per item (per-unit encoding).
                        # AI lists how many of this material a SINGLE unit needs via repetition
                        # (e.g., ["moving_blanket","moving_blanket"] = 2 blankets PER sofa).
                        # Multiply by item.quantity to get total across all units.
    ...
    #
    # Base person-hours per room size (4-person crew packing at normal density):
    #   small (bath/closet): ~1.0 ph  |  large (bedroom/kitchen): ~2.5 ph
    #   xlarge (master/basement): ~4.0 ph
    ...
    def _classify_room_content(self, items: List[Any]) -> Dict[str, bool]:
        ...
        Returns flags like fragile_heavy, furniture_heavy, etc.
        Thresholds: a type is "heavy" if ≥30% of unique item lines are that type.
        """
        if not items:
            return {}
        ...
        flags = {}
        threshold = max(1, total_lines * 0.30)
    ...
    def classify_labor_hours(
        ...
        for room in rooms:
            ...
            # If density is light and room is small, content drives time more
            # than room size. Cap base at 0.5 ph for light+small.
            if room.density == "light" and room_size == "small":
                base_ph = 0.5

            # Apply density (already captures "how full the room is")
            room_ph = base_ph * density_mult * floor_mult * contamination_mult

            # Content-type modifiers (what kind of stuff → how careful/slow)
            ...
            if content_flags.get("fragile_heavy"):
                # A fragile-heavy room: all packing time is fragile-rate work
                fragile_mins += room_mins
            elif content_flags.get("electronics_heavy"):
                # Electronics require careful handling → specialty rate
                specialty_mins += room_mins * 0.50
                standard_mins += room_mins * 0.50
            ...
            # High-value items in the room → small specialty surcharge
            hv_count = sum(1 for i in items if getattr(i, 'is_high_value', False))
            if hv_count > 0:
                # ~10 min per high-value item for documentation (serial#, appraisal photo)
    ...
    def calculate_estimate_from_content(self, request: RoomsEstimateRequest) -> EstimateResponse:
        ...
        # Insurance claims require detailed photo + written inventory per item → ~9% of total labor
        inventory_hours = rh(total_labor_hours * 0.09) if total_labor_hours > 2 else 0.5
        # Supervisor: 1 person, not full crew
        # Xactimate CPS BXSME supervisor component = ~19% of pack labor; 15% is conservative
        supervisor_hours = rh(total_labor_hours * 0.15) if total_labor_hours > 2 else 0.5

        # Pack-out elapsed hours — use actual calculated values, no artificial minimums
        # that inflate small jobs. Round to 0.5-step for billing.
        ...
        hv_count = sum(
            item.quantity for room in request.rooms for item in room.items
            if getattr(item, "is_high_value", False)
        )
        supplements = self.evaluate_supplements(
            request.rooms, subtotal,
        ...
        if po_standard_hrs > 0:
            ...
            po_lines.append({"name": "Standard Pack-Out", "qty": ph, "unit": "HR",
                "rate": round(labor_rate, 2),
        ...
        if po_fragile_hrs > 0:
            ...
            po_lines.append({"name": "Fragile / High-Care Items", "qty": ph, "unit": "HR",
        ...
        if po_specialty_hrs > 0:
            ...
            po_lines.append({"name": "Specialty / High-Value Items", "qty": ph, "unit": "HR",
        ...
        if po_furniture_hrs > 0:
            ...
            po_lines.append({"name": "Furniture Disassembly", "qty": ph, "unit": "HR",
                "rate": round(labor_rate, 2),
        ...
        if po_appliance_hrs > 0:
            ...
            po_lines.append({"name": "Appliance Handling", "qty": ph, "unit": "HR",
                "rate": round(labor_rate, 2),
                "detail": f"{po_appliance_hrs} elapsed hr · {crew}-person crew  (disconnect, secure internals, dolly)",
                "amount": round(ph * labor_rate, 2)})
        if inventory_hours > 0:
            po_lines.append({"name": "Inventory & Documentation", "qty": float(inventory_hours), "unit": "HR",
                "rate": round(labor_rate, 2),
        ...
        if supervisor_hours > 0:
            po_lines.append({"name": "Supervision", "qty": float(supervisor_hours), "unit": "HR",
                "rate": round(fragile_rate, 2),
        ...
        if request.include_packback:
            ...
            if pb_standard_hrs > 0:
                ...
                pb_lines.append({"name": "Standard Pack-Back", "qty": ph, "unit": "HR",
                    "rate": round(labor_rate, 2),
            ...
            if pb_appliance_hrs > 0:
                ...
                pb_lines.append({"name": "Appliance Reconnection", "qty": ph, "unit": "HR",
                    "rate": round(labor_rate, 2),
                    "detail": f"{pb_appliance_hrs} elapsed hr · {crew}-person crew  (reconnect utilities, test operation)",
                    "amount": round(ph * labor_rate, 2)})
            if pb_supervisor > 0:
                pb_lines.append({"name": "Supervision", "qty": float(pb_supervisor), "unit": "HR",
                    "rate": round(fragile_rate, 2),
    ...
    def get_prices_dict(self) -> Dict[str, float]:
        ...
        for code, name in CODE_TO_NAME.items():
            if code in self.prices:
                result[name] = self.prices[code].price
        # Material key-based prices
        for key, code in MATERIAL_CODES.items():
            if code in self.prices:
                result[key] = self.prices[code].price
        return result


def get_calculator(db: Session) -> EstimateCalculator:
    """Factory function to create calculator instance"""
    return EstimateCalculator(db)
