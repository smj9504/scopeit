"""
Export Service - Professional Estimate Format
Matching SOS Moving Company estimate style exactly
"""
...
def generate_estimate_pdf(
    ...
    # Use frontend line_items when present (exact UI line items), otherwise recalculate
    ...
    supplements_total = sum(s.get('amount', 0) for s in supplements if s.get('enabled', True))
    subtotal = sum(item['qty'] * item['price'] for section in sections for item in section['items'])
    op_amount = estimate_data.get('op_amount') if estimate_data.get('op_amount') is not None else (subtotal * (op_rate / 100) if include_op else 0)
    contingency_amount = estimate_data.get('contingency_amount') if estimate_data.get('contingency_amount') is not None else (subtotal * (contingency_rate / 100) if include_contingency else 0)
    tax_amount = (subtotal + op_amount + contingency_amount + supplements_total) * (tax_rate / 100) if tax_rate > 0 else 0
    grand_total = subtotal + op_amount + contingency_amount + supplements_total + tax_amount
...
Estimate date: {estimate_date}</font>""", 
    ...
    if include_contingency:
        totals_data.append([Paragraph(f'Contingency ({contingency_rate}%)', style_right), Paragraph(f"${contingency_amount:,.2f}", style_right)])
    for supp in supplements:
        if supp.get('enabled', True) and supp.get('amount', 0) > 0:
            supp_name = supp.get('name', 'Supplement')
            supp_amt = supp.get('amount', 0)
            totals_data.append([Paragraph(supp_name, style_right), Paragraph(f"${supp_amt:,.2f}", style_right)])
    if tax_rate > 0:
        totals_data.append([Paragraph('Tax', style_right), Paragraph(f"${tax_amount:,.2f}", style_right)])
    totals_data.append([
        Paragraph('<b>Total Estimate</b>', ParagraphStyle('R', fontSize=10, fontName='Helvetica-Bold', alignment=TA_RIGHT)),
        Paragraph(f"<b>${grand_total:,.2f}</b>", ParagraphStyle('R', fontSize=10, fontName='Helvetica-Bold', alignment=TA_RIGHT))
    ])
...
def generate_estimate_excel(
    ...
    if include_contingency:
        ws.cell(row=row, column=4, value=f'Contingency ({contingency_rate}%)').font = normal_font
        ws.cell(row=row, column=4).alignment = Alignment(horizontal='right')
    ...
    for supp in supplements:
        if supp.get('enabled', True) and supp.get('amount', 0) > 0:
            ws.cell(row=row, column=4, value=supp.get('name', 'Supplement')).font = normal_font
            ws.cell(row=row, column=4).alignment = Alignment(horizontal='right')
            ws.cell(row=row, column=5, value=supp.get('amount', 0)).font = normal_font
            ws.cell(row=row, column=5).number_format = '$#,##0.00'
            row += 1
